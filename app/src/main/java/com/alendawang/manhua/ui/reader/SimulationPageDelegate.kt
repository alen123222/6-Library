...
/**
 * 仿真翻页（卷曲效果）
 * 移植自 Legado 的 SimulationPageDelegate
 * 使用贝塞尔曲线模拟真实书页翻转
 */
...
class SimulationPageDelegate(pageView: PageView) : HorizontalPageDelegate(pageView) {
    ...
    override fun setBitmap() {
        ...
        // Also call super to set parent bitmaps
        super.setBitmap()
    }
    ...
    override fun onTouch(event: MotionEvent) {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                } else if (startY > viewHeight / 3 && startY < viewHeight / 2
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (mDirection) {
            PageDirection.NEXT -> {
                calcPoints()
                drawCurrentPageArea(canvas, simCurBitmap)
                drawNextPageAreaAndShadow(canvas, simNextBitmap)
                drawCurrentPageShadow(canvas)
                drawCurrentBackArea(canvas, simCurBitmap)
            }
            PageDirection.PREV -> {
                calcPoints()
                drawCurrentPageArea(canvas, simPrevBitmap)
                drawNextPageAreaAndShadow(canvas, simCurBitmap)
                drawCurrentPageShadow(canvas)
                drawCurrentBackArea(canvas, simPrevBitmap)
            }
            else -> return
        }
    }
    ...
    private fun calcPoints() {
        ...
        mBezierEnd1 = getCross(
            PointF(mTouchX, mTouchY), mBezierControl1, mBezierStart1, mBezierStart2
        )
        mBezierEnd2 = getCross(
            PointF(mTouchX, mTouchY), mBezierControl2, mBezierStart1, mBezierStart2
        )
