            ...
            try {
                when (currentMediaType) {
                    MediaType.COMIC -> {
                        ...
                        scanComicsFlow(context, treeUri, existingComicsMap, totalItemCount, nomediaEnabled = loadNomediaEnabled(context)) { name ->
                    ...
                    MediaType.AUDIO -> {
                        ...
                        scanAudiosFlow(context, treeUri, existingUris, totalItemCount, nomediaEnabled = loadNomediaEnabled(context)) { name ->
                            if (scanState.isScanning) scanState = scanState.copy(currentFolder = name)
                        }.collect { newAudio ->
